...
Идёт 2163 год. Мишу, который работает в отделении таможни при космодроме города
Нью-Питер, вызвал в кабинет шеф.
...
Формат ввода
На первой строке входного файла задано число N (0 ≤ N ≤ 50 000). На следующих N
строках находится по 2 целых положительных числа Ti и Li – время прибытия
соответствующего груза и время,
...
# first finish shipment inspection, then start new one
START = 1
END = -1


def findmachinesnum(times):
    events = []
    for t in times:
        events.append((t[0], START))
        events.append((t[0] + t[1], END))
    events.sort()
    ...
    for ev in events:
        if ev[1] == START:
            packagescnt += 1
            maxcnt = max(maxcnt, packagescnt)
        else:
            packagescnt -= 1
    return maxcnt


print(findmachinesnum(times))
