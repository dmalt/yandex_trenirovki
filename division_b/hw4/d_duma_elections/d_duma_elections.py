...
“первое избирательное частное” (смысл первого избирательного
частного - это количество голосов избирателей, которое необходимо набрать
...
пробелы), затем, через пробел, количество голосов,
...
with open("input.txt", "r") as f:
    for line in f:
        ...
        nowvotes = int(tmp[-1])
        votes.append(nowvotes)
        total_votes += nowvotes
        names.append(" ".join(tmp[:-1]))


first_el_quotent = total_votes / 450
div_votes = [int(v // first_el_quotent) for v in votes]

total_div_votes = sum(div_votes)

if total_div_votes < 450:
    remaining = 450 - total_div_votes

    ind_sort_by_remainder = sorted(
        zip(votes, range(len(votes))),
        key=lambda x: (x[0] % first_el_quotent, x[0]),
        reverse=True,
    )
    for _, i in ind_sort_by_remainder:
        if remaining:
            div_votes[i] += 1
            remaining -= 1
        else:
...
for n, v in zip(names, div_votes):
    print(n, v)
