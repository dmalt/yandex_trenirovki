...
магазина. Расстояние между двумя соседними домами считается равным 1
(то есть если два дома стоят рядом, то между ними расстояние 1, если между
...
last_store_ind = float('-inf')
current_houses = []
maxdist = float('-inf')

for i, num in enumerate(nums):
    if num == STORE:
        while current_houses:
            h = current_houses.pop()
            # first store occurence is handled as well
            maxdist = max(maxdist, min(h - last_store_ind, i - h))
        last_store_ind = i
    elif num == HOUSE:
        current_houses.append(i)

if current_houses:
    h = current_houses.pop()
    maxdist = max(maxdist, h - last_store_ind)
