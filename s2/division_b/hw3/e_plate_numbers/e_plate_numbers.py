...
цифры номера. Но при этом свидетели не помнят порядок этих цифр и букв. Полиция
хочет проверить несколько подозреваемых автомобилей. Будем говорить, что номер
...
Формат ввода
Сначала задано число  - количество свидетелей. Далее идет M строк, каждая из
которых описывает показания очередного свидетеля. Эти строки непустые и состоят
из не более чем 20 символов. Каждый символ в строке - либо цифра, либо
...
testimony = []
for i in range(M):
    testimony.append(set(input().strip()))
N = int(input().strip())
plates = []
for i in range(N):
    plates.append(input().strip())


def count_coincedences(plate):
    count = 0
    plate_chars = set(plate)
    for t in testimony:
        if t <= plate_chars:
            count += 1
    return count
...
for i, plate in enumerate(plates):
    c = count_coincedences(plate)
    coincidences[i] = c
    max_coinc = max(max_coinc, c)

for i, plate in enumerate(plates):
    if coincidences[i] == max_coinc:
        print(plate)
